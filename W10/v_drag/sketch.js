...
function setup() {
  ...
  button = createButton('Click to start the game!');
  button.position(width/2-250, height/2-100);
  button.mousePressed(startGame);
  button.size(500,100);
  button.style('font-size', '20px');
  button.style('background-color', "#ff4929");
...
if(gameStart){  
  ...
  //print(label == undefined);
        ...
        if(label != undefined && confidence > 2){
          ...
          decrease = 20;
          a1 = 0;
          let speedDis = map(confidence, 0, 5, 0, 300);
...
function gotResult(error, results) {
  ...
  if(gameStart){
    ...
    confidence = results.find(isCar).confidence * 5;
    print(confidence);
  }
}
